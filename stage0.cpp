		...
		if ((!initialized) && (lpWindowName != NULL)) {
			...
			if (res == 0) {
				...
				toml::table tbl;
				{
					...
					tbl.emplace("enabled_mods", toml::array());
					tbl.emplace("show_console", false);
					tbl.emplace("skip_ui", false);
				}

				boost::container::flat_set<std::string> loaderMods;
				...
				if (skip_ui) {
					...
					bool windowClosed = gui::WinMain(hInstance, &mods, &loaderMods);
					if (!windowClosed) {
						exit(0);
					}
				}
